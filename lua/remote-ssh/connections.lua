...
M.unmount_host = function()
    -- Stoppe den SSHFS-Prozess, falls er läuft
    if sshfs_job_id then
        local stopped = vim.fn.jobstop(sshfs_job_id)
        if stopped == 0 then
            vim.notify("SSHFS process stopped successfully.")
        else
            vim.notify("Failed to stop SSHFS process (Error code: " .. stopped .. ").", vim.log.levels.WARN)
        end
        sshfs_job_id = nil
    end

    -- Falls ein Mount-Punkt existiert, versuche ihn auszuhängen
    if mount_point then
        vim.notify("Unmounting " .. mount_point)
        -- Auf macOS benutzt du typischerweise 'diskutil unmount'
        local unmount_cmd = 'diskutil unmount "' .. mount_point .. '"'
        local result = vim.fn.system(unmount_cmd)
        -- Optional: Prüfe das Ergebnis und gib eine Rückmeldung
        if string.find(result, "successful") or string.find(result, "Unmount successful") then
            vim.notify("Unmounted " .. mount_point .. " successfully.")
        else
            vim.notify("Unmount failed: " .. result, vim.log.levels.ERROR)
        end
        mount_point = nil
        current_host = nil
    else
        vim.notify("No mount point found to unmount.")
    end
end
